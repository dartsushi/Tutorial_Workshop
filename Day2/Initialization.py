...
def initialize_Ising(T):
    beta = 1./T
    Ising=np.zeros((2,2,2,2))
    for i,si in enumerate(range(-1,2,2)):
        for j,sj in enumerate(range(-1,2,2)):
            for k,sk in enumerate(range(-1,2,2)):
                for l,sl in enumerate(range(-1,2,2)):
                    sum_prod = si*sj+sj*sk+sk*sl+sl*si
                    Ising[i,j,k,l]=np.exp(beta*sum_prod)
    return Ising

def impurity_Ising(T):
    beta = 1./T
    Ising=np.zeros((2,2,2,2))
    for i,si in enumerate(range(-1,2,2)):
        for j,sj in enumerate(range(-1,2,2)):
            for k,sk in enumerate(range(-1,2,2)):
                for l,sl in enumerate(range(-1,2,2)):
                    sum_prod = si*sj+sj*sk+sk*sl+sl*si
                    sum_spin = si+sj+sk+sl
                    Ising[i,j,k,l]= sum_spin*np.exp(beta*sum_prod)/4
    return Ising
...
def e_ini_Ising(T):
    ...
    for i,si in enumerate(range(-1,2,2)):
        for j,sj in enumerate(range(-1,2,2)):
            for k,sk in enumerate(range(-1,2,2)):
                sum_prod = si*sj+sj*sk
                e[i,j,k]= np.exp(beta*sum_prod)
    return e
